...
void get_obj(int flag)
{
  int a = flag;
  FILE* fp;
  if(a==1)
  {
  	fp = fopen("sobj1.txt","r");
  }
  else if(a==2)
  {
  	fp = fopen("sobj2.txt","r");
  }
  else if(a==3)
  {
  	fp = fopen("sobj3.txt","r");
  }
  ...
  if((1.0-value)<0.001)
    {
      obj[a][0] = fr*(1-fr)*value + fr*fr;
      for(int i=1; i<Nt; i++)
	{
	  fscanf(fp, "%d", &ind);
	  fscanf(fp, "%f", &value);

	   obj[a][i]=fr*(1-fr)*value + fr*fr;
	}
...
double energy(double S[3][Nt])
{
  ...
  for(int i=1; i<Nt; i++)
    {
      E = E + (S[a][i] - obj[a][i])*(S[a][i] - obj[a][i]);
    }
   }  
...
double d_energy(double S2[3][Nt], double ST2[3][Nt])
{
  double d_E = 0;
...
main()
{
  ...
  for(int i=0; i<MAXX; i++)
	...
	for(int r=0; r<Nt; r++)
	  {
	    SS2[0][r] += lineS2[i][j][r];
	    SS2[1][r] += columeS2[i][j][r];
	    SS2[2][r] += heightS2[i][j][r];
	  }
  ...
  for(int r=0; r<Nt; r++)
    {
      S2[0][r] = (double) SS2[0][r]/(double)(MAXX*MAXX*MAXX);
      S2[1][r] = (double) SS2[1][r]/(double)(MAXX*MAXX*MAXX);
      S2[2][r] = (double) SS2[2][r]/(double)(MAXX*MAXX*MAXX);
      //printf("%d \t  %f \n", r, S2[r]);
    }
       ...
       for(int i=0; i< Nevl; i++)
	 {
	   ...
	   for(int r=0; r<Nt; r++)
	     {       
	       ...
	       //************************************************
	       TEMP[0][r] = 0; //old value
		   TEMP[1][r] = 0;
		   TEMP[2][r] = 0;
		    
	       LCHV[0][r] = 0; //current value
	       LCHV[1][r] = 0;
	       LCHV[2][r] = 0;
	       ...
	       for(int vv=0; vv<2; vv++)
		 {
	       ...
	       LCHV[0][r] = lineS2[indexj][indexk][r]+lineS2[indexn][indexl][r];
	       
		   LCHV[1][r] = columeS2[indexi][indexk][r]+columeS2[indexm][indexl][r];
		   
		   LCHV[2][r] = heightS2[indexi][indexj][r]+heightS2[indexm][indexn][r];
	       ...
	       if(r!=0)
		 {
	       ...
	       */
	       for(int a=0; a<3; a++)
	       {
	        ...
	        SS2[a][r] = (int)(SS2[a][r] - TEMP[a][r] + LCHV[a][r]);
	       
	       ST2[a][r] = (double)SS2[a][r]/(double)(MAXX*MAXX*MAXX);
           }
      ...
      } //this is the end of one temperature stage
